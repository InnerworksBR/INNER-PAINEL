# Inner Agent - Script de Monitoramento v1.0.2
# Copie este arquivo para o servidor e renomeie para agent.ps1
...
function Get-HostMetrics {
    try {
        # Coletar CPU - com fallback se Get-Counter falhar
        $cpuValue = 0
        try {
            ...
            Write-Log "Get-Counter falhou, usando metodo alternativo" "WARN"
            # Fallback: usar WMI
            $cpu = Get-CimInstance Win32_Processor | Select-Object -First 1
            if ($cpu) { $cpuValue = $cpu.LoadPercentage }
        }
        ...
        $totalDisk = if ($disk.Size) { [math]::Round($disk.Size / 1GB, 2) } else { 0 }
        $freeDisk = if ($disk.FreeSpace) { [math]::Round($disk.FreeSpace / 1GB, 2) } else { 0 }
        $usedDisk = $totalDisk - $freeDisk
        $diskPercent = if ($totalDisk -gt 0) { [math]::Round(($usedDisk / $totalDisk) * 100, 2) } else { 0 }
